...
Tìm kiếm dùng `collection.php?search=...&category=...`. Thao tác yêu thích dùng POST tới `favorite-action.php`, có CSRF token và redirect về trang trước đó.
...
| `app/login/page.tsx` + login API | `login.php`, `logout.php` |
| `app/dashboard/page.tsx` | `dashboard.php` |
| Dashboard collection/favorites state | `collection.php`, `favorites.php`, `favorite-action.php` |
...
```powershell
C:\xampp\php\php.exe -S 127.0.0.1:8090 -t php-version
```
