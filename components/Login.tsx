          ...
          <div style={{ width: "100%", maxWidth: "26rem" }}>
            ...
            <form onSubmit={handleSubmit} aria-label="Đăng nhập" aria-busy={loading}>
              <div className="mb-3"><label htmlFor="email" className="form-label">Email</label>
                <input id="email" name="email" type="email" autoComplete="username" className="form-control" placeholder="ban@vidu.com" required aria-invalid={Boolean(status)} aria-describedby={status ? "login-status" : undefined} value={email} onChange={e => { setEmail(e.target.value); setStatus(null); }} /></div>
              <div className="mb-3"><label htmlFor="password" className="form-label">Mật khẩu</label>
                <input id="password" name="password" type={showPassword ? "text" : "password"} autoComplete="current-password" className="form-control" placeholder="Nhập mật khẩu" required aria-invalid={Boolean(status)} aria-describedby={status ? "login-status" : undefined} value={password} onChange={e => { setPassword(e.target.value); setStatus(null); }} /></div>
              ...
              {status && <div id="login-status" className="alert alert-danger py-2 small" role="alert">{status}</div>}
              <button type="submit" className="btn btn-brand w-100 py-3 mb-3" disabled={loading}>{loading ? "Đang kiểm tra demo…" : "Đăng nhập demo →"}</button>
            </form>
            <p className="demo-note mt-3 mb-4">Bản demo giao diện · không tạo phiên đăng nhập thật.<br />Email: <strong>demo@dv03.vn</strong><br />Mật khẩu: <strong>123456</strong></p>
            <Link href="/dashboard" className="link-brand small">Xem catalog không cần đăng nhập →</Link>
          </div>
        </div>
      </div>
    </main>
